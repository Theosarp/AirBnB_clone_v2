...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User name="Kofi" age=31 height=5.3')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Kofi'", cout.getvalue().strip())
            self.assertIn("'age': 31", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="theo.sarp@yahoo.com" password="123"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
            ...
            self.assertIn('theo.sarp@yahoo.com', result)
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="theo.sarp@yahoo.com", password="123")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('theo.sarp@yahoo.com', result)
            self.assertIn('123', result)
            self.assertIn('theo.sarp@yahoo.com', cout.getvalue())
